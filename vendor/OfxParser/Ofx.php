...
class Ofx
{
    ...
    private function buildSignOn(SimpleXMLElement $xml)
    {
        ...
        // $signOn->date = $this->createDateTimeFromStr($xml->DTSERVER, true); // EXCLUÍDO
        $signOn->date = $xml->DTSERVER;
    ...
    private function buildBankAccount($transactionUid, SimpleXMLElement $statementResponse)
    {
        ...
        // $bankAccount->balanceDate = $this->createDateTimeFromStr( // EXCLUÍDO
        //     $statementResponse->LEDGERBAL->DTASOF,
        //     true
        // );
        $bankAccount->balanceDate = $statementResponse->LEDGERBAL->DTASOF;

        $bankAccount->statement = new Statement();
        $bankAccount->statement->currency = $statementResponse->CURDEF;

        // $bankAccount->statement->startDate = $this->createDateTimeFromStr( // EXCLUÍDO
        //     $statementResponse->BANKTRANLIST->DTSTART
        // );
        $bankAccount->statement->startDate = $statementResponse->BANKTRANLIST->DTSTART;

        // $bankAccount->statement->endDate = $this->createDateTimeFromStr( // EXCLUÍDO
        //     $statementResponse->BANKTRANLIST->DTEND
        // );
        $bankAccount->statement->endDate = $statementResponse->BANKTRANLIST->DTEND;
    ...
    private function buildCreditAccount(SimpleXMLElement $xml)
    {
        ...
        // $creditAccount->balanceDate = $this->createDateTimeFromStr($xml->CCSTMTRS->LEDGERBAL->DTASOF, true); // EXCLUÍDO
        $creditAccount->balanceDate = $xml->CCSTMTRS->LEDGERBAL->DTASOF;

        $creditAccount->statement = new Statement();
        $creditAccount->statement->currency = $xml->CCSTMTRS->CURDEF;
        // $creditAccount->statement->startDate = $this->createDateTimeFromStr($xml->CCSTMTRS->BANKTRANLIST->DTSTART); // EXCLUÍDO
        $creditAccount->statement->startDate = $xml->CCSTMTRS->BANKTRANLIST->DTSTART;
        // $creditAccount->statement->endDate = $this->createDateTimeFromStr($xml->CCSTMTRS->BANKTRANLIST->DTEND); // EXCLUÍDO
        $creditAccount->statement->endDate = $xml->CCSTMTRS->BANKTRANLIST->DTEND;
    ...
    private function buildTransactions(SimpleXMLElement $transactions)
    {
        ...
        foreach ($transactions as $t) {
            ...
            // $transaction->date = $this->createDateTimeFromStr($t->DTPOSTED); // EXCLUÍDO
            $transaction->date = (string)$t->DTPOSTED;
            if ('' !== (string)$t->DTUSER) {
                // $transaction->userInitiatedDate = $this->createDateTimeFromStr($t->DTUSER); // EXCLUÍDO
                $transaction->userInitiatedDate = $t->DTUSER;
            }
